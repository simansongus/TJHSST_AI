...
def quickMove(brd, token):
    ...
    for p in pos:
        ...
        if p in [8,16,24,32,40,48,15,23,31,39,47,55,1,2,3,4,5,6,57,58,59,60,61,62] and canBeRecap(brd, p, token) == False:
            ...
            oppMoves = len(posMoves(placeMove(brd, p, token), "xo".replace(token, "")))
            if oppMoves < best[0]:
                best = oppMoves, p
                continue
    return best[1]

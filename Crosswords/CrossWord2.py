...
def setGlobals(args):
    ...
    global big; big = False
    for word in wordList:
        if len(word) >= 3:
            for idx, val in enumerate(word):
                if (len(word), idx, val) not in dictWord:
                    dictWord[(len(word), idx, val)]  = {word}
                else:
                    dictWord[(len(word), idx, val)].add(word)
            if len(word) not in dictWordLength: dictWordLength[len(word)] = {word}
            else: dictWordLength[len(word)].add(word)
        else: continue

    global height; height = int(args[1][:args[1].index("x")])
    global width; width = int(args[1][args[1].index("x")+1:])
    #if width*height >  500: big = True
...
def bruteForce(pzl,numBlocks, lastIdx):
    ...
    if big:
        pos = basicposMovesCached(pzl)
    else: pos = posMovesCached(pzl)
...
def isPlacedInvalid(pzl, placedWords, startIdx):
    ...
    for st in containedStarts:
        pos = posWordsCached(st, pzl, placedWords)
        if not pos or len(pos[1]) == 0:
            return True
    return False

POSWORDSCACHE = {}
def posWordsCached(start, puzzle, placedWords):
    #placedWordsTuple = tuple(sorted(placedWords))
    key = (start, puzzle)
    if key in POSWORDSCACHE:
        return POSWORDSCACHE[key]
    else:
        temp = posWords(start, puzzle, placedWords)
        POSWORDSCACHE[key] = temp
        return temp
    
def posWords(start, puzzle, placedWords):
    idxList = startToIdxsDictionary[start]
    section = "".join([puzzle[idx] for idx in idxList])
    if "-" not in section and section.lower() in wordList:
        return True, [section]
    ...
    if len(contraList) == 0:
        return True, [word for word in dictWordLength[length] if word not in placedWords]
    if (length, contraList[0][0], contraList[0][1]) not in dictWord:
        return False, []
    posWords = dictWord[length, contraList[0][0], contraList[0][1]]
    for x in contraList[1:]:
        if (length, x[0], x[1]) not in dictWord:
            return False, []
        posWords = posWords.intersection(dictWord[length, x[0], x[1]])
    posWords = [word for word in posWords if word not in placedWords]
    if len(posWords) == 0:
        return False, []
    return True, posWords
...
def solvePuzzle(pzl, numBlocks, startIdx, placedWords):
    ...
    pWords = posWordsCached(start, pzl, placedWords)

...
word1 = "scared"
word2 = "seared"
wordList = open("laddertester.txt").read().splitlines()
...
if second_degree_word is not None:
    print("Second degree word: " + secondDegree(degrees))
else:
...
"""
Connected component size count: The number of distinct sizes of connected components.
Largest component size: The number of vertices in the largest connected component
K2 count: The number of pairs of vertices adjacent only to each other
K3 count: The number of triples of vertices each adjacent to both of, and only, the other two (eg. anion, onion, union)
K4 count: The number of quadruples of vertices each adjacent to all of, and only, the other three
Neighbors: All the neighbors of word1
Farthest: A word that is farthest from word1
Path: A shortest path between word1 and word2 """
...
'''   k3 = 0
    ...
    for size in component_sizes:
        if size == 3:
            if all(len(edgeList[wordList.index(node)]) == 2 for node in component_sizes):
                k3 += 1
        elif size == 4:
            if all(len(edgeList[wordList.index(node)]) == 3 for node in component_sizes):
                k4 += 1 '''